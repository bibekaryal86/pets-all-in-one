...
import static pets.database.utils.Commons.convertDtoToEntityRefMerchants;
...
import pets.database.repository.RefMerchantRepository;
import pets.models.model.RefMerchantRequest;
import pets.models.model.RefMerchantResponse;
import pets.models.model.Status;

@Service
public class RefMerchantServiceDb {
    private static final Logger logger = LoggerFactory.getLogger(RefMerchantServiceDb.class);

    private final RefMerchantRepository refMerchantDao;

    public RefMerchantServiceDb(RefMerchantRepository refMerchantDao) {
        this.refMerchantDao = refMerchantDao;
    }

    public RefMerchantResponse getAllRefMerchants() {
        logger.info("Before Get All Ref Merchants");
        List<RefMerchantDto> refMerchants = new ArrayList<>();
        Status status = null;
        ...
        return RefMerchantResponse.builder()
                .refMerchants(convertDtoToEntityRefMerchants(refMerchants))
                .status(status)
                .build();
    }
    ...
    public RefMerchantResponse getRefMerchantById(String id) {
        ...
        RefMerchantDto refMerchant = null;
        Status status = null;
        ...
        return RefMerchantResponse.builder()
                .refMerchants(refMerchant == null ? emptyList() : convertDtoToEntityRefMerchants(singletonList(refMerchant)))
                .status(status)
                .build();
    }
    ...
    public RefMerchantResponse saveNewRefMerchant(RefMerchantRequest refMerchantRequest) {
        ...
        RefMerchantDto newRefMerchant;
        Status status = null;

        try {
            newRefMerchant = RefMerchantDto.builder()
        ...
        return RefMerchantResponse.builder()
                .refMerchants(newRefMerchant == null ? emptyList() : convertDtoToEntityRefMerchants(singletonList(newRefMerchant)))
                .status(status)
                .build();
    }
