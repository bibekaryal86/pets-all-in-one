...
import static pets.database.utils.Commons.convertDtoToEntityRefBanks;
...
import pets.database.repository.RefBankRepository;
import pets.models.model.RefBankRequest;
import pets.models.model.RefBankResponse;
import pets.models.model.Status;

@Service
public class RefBankServiceDb {
    private static final Logger logger = LoggerFactory.getLogger(RefBankServiceDb.class);

    private final RefBankRepository refBankDao;

    public RefBankServiceDb(RefBankRepository refBankDao) {
        this.refBankDao = refBankDao;
    }

    public RefBankResponse getAllRefBanks() {
        logger.info("Before Get All Ref Banks");
        List<RefBankDto> refBanks = new ArrayList<>();
        Status status = null;
        ...
        return RefBankResponse.builder()
                .refBanks(convertDtoToEntityRefBanks(refBanks))
                .status(status)
                .build();
    }
    ...
    public RefBankResponse getRefBankById(String id) {
        ...
        return RefBankResponse.builder()
                .refBanks(refBank == null ? emptyList() : convertDtoToEntityRefBanks(singletonList(refBank)))
                .status(status)
                .build();
    }
    ...
    public RefBankResponse saveNewRefBank(RefBankRequest refBankRequest) {
        ...
        RefBankDto newRefBank;
        Status status = null;

        try {
            newRefBank = RefBankDto.builder()
        ...
        return RefBankResponse.builder()
                .refBanks(newRefBank == null ? emptyList() : convertDtoToEntityRefBanks(singletonList(newRefBank)))
                .status(status)
                .build();
    }
